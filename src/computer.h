...
enum OpType { TEST, LABEL, JUMP };

// Przykładowa operacja
template <int a, int b> struct TestOp {
    template <size_t memSize, typename memType>
    static constexpr void execute(std::array<memType, memSize>& mem, bool& ZF,
                                  bool& SF) {
        mem[a] = b;
    }
...
template <typename Label, typename Program> struct LabelHolder {
    using program = Program;
    using label = Label;
};

template <uint64_t Id> struct Label {
...
template <uint64_t Id> struct Jump {

    template <size_t memSize, typename memType, typename labels>
    static constexpr void execute(std::array<memType, memSize>& mem, bool& ZF,
                                  bool& SF) {
        labels::template find_and_run<memSize, memType, labels>(Id, mem, ZF,
                                                                SF);
    }
...
//----------------HANDLING LABELS-------------------
template <typename... Labels> struct LabelList;
...
template <typename... Ops> struct Program {
    template <size_t memSize, typename memType, typename labels>
    static constexpr void run(std::array<memType, memSize>& mem, bool& ZF,
                              bool& SF);
};
...
template <typename Op, typename... Ops> struct Program<Op, Ops...> {
    ...
    static constexpr void run(std::array<memType, memSize>& mem, bool& ZF,
        ...
        default:
            ...
            Program<Ops...>::template run<memSize, memType, labels>(mem, ZF,
                                                                    SF);
            break;
        }
    }
...
template <int N> struct Num { constexpr static int value = N; };
